...
const TaskStats = ({ tasks, stats }) => {
  // Use API stats if provided, otherwise calculate from tasks
  ...
  const inProgressTasks = stats?.inProgressTasks || tasks.filter(t => t.status === 'in-progress').length;
  const overdueTasks = stats?.overdueTasks || tasks.filter(t => {
    const dueDate = new Date(t.dueDate);
    const today = new Date();
    return dueDate < today && t.status !== 'completed';
  }).length;
